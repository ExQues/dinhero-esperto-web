  ...
  return (
    <footer className="bg-slate-900 text-slate-400 border-t border-slate-800">
      <div className="container mx-auto py-12 md:py-16 px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-8 mb-10 md:mb-12">
          ...
          {footerSections.map((section) => (
            <div key={section.title}>
              <h3 className="text-slate-200 font-semibold text-md mb-5">{section.title}</h3>
              <ul className="space-y-3">
                {section.links.map((link) => (
                  <li key={link.name}>
                    <a 
                      href={link.href} 
                      className="text-sm text-slate-400 hover:text-sky-400 hover:underline transition-colors duration-200"
                    >
                      {link.name}
                    </a>
                  </li>
        ...
        <div className="mt-10 pt-8 border-t border-slate-800 text-center text-slate-500 text-xs">
