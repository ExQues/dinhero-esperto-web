...
const pricingPlans = [
  ...
  },
  {
    ...
    cardClass: 'bg-gradient-to-br from-slate-700 via-slate-800 to-slate-700 border-amber-500 scale-105'
  }
];
...
const Pricing = () => {
  ...
  const handleUpgrade = async () => {
    ...
    // Idealmente, usar um sistema de notificação mais robusto (como o Sonner já instalado)
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold mb-6 leading-tight">
            Planos <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-blue-500">flexíveis</span> para suas necessidades
          </h2>
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-10 max-w-4xl mx-auto items-stretch">
          {pricingPlans.map((plan, index) => (
            <div 
              key={index}
              className={`rounded-xl overflow-hidden shadow-2xl border flex flex-col ${plan.cardClass} ${plan.highlight ? 'relative' : ''}`}
            >
              {plan.highlight && (
              ...
              <div className="p-6 md:p-8 flex flex-col flex-grow">
                <h3 className={`text-2xl font-bold mb-2 ${plan.highlight ? 'text-amber-300' : 'text-sky-400'}`}>{plan.title}</h3>
                <div className="mb-5">
                  <span className={`text-4xl font-extrabold ${plan.highlight ? 'text-white' : 'text-slate-100'}`}>{plan.price}</span>
                  {plan.pricePeriod && <span className="text-slate-400 ml-1 text-lg">{plan.pricePeriod}</span>}
                </div>
                ...
                <ul className="space-y-3 mb-8 flex-grow">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-start">
                      <Check className={`h-5 w-5 ${plan.highlight ? 'text-amber-400' : 'text-sky-500'} mr-2.5 mt-0.5 flex-shrink-0`} />
